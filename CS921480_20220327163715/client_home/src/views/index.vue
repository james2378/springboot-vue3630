		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/wonderful_video/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_wonderful_video = !show_list_wonderful_video">
		                        <div class="switch_box">
		                            <span v-if="show_list_wonderful_video"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--精彩视频推荐列表组件 -->
		                <list_wonderful_video v-if="show_list_wonderful_video" :list="list_wonderful_video" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_wonderful_video.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_wonderful_video" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_wonderful_video" :key="i" @click="to_details('wonderful_video',o,'wonderful_video_id')">
		                            <td v-for="(oj,n) in fields_wonderful_video">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_wonderful_video: true,
				list_wonderful_video: [],
				vm_arr_wonderful_video: [
					"wonderful_video_id",
					"video_number",
					"video_name",
					"video_category",
		...
		methods: {
			...
			// 获取精彩视频列表
			get_wonderful_video() {
				let url = "~/api/wonderful_video/get_list?";
				this.$get(url, {
					...
					if (json.result) {
						this.list_wonderful_video = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_wonderful_video() {
		        var list = this.list_wonderful_video;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_wonderful_video.map((o) => {
		                // 第二个中括号是对象的属性
