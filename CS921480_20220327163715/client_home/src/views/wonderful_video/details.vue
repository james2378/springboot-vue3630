<template>
	<div class="diy_details page_wonderful_video" id="wonderful_video_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_wonderful_video'>
							<div_wonderful_video :obj="obj"></div_wonderful_video>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_wonderful_video from "@/components/diy/div_wonderful_video.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "wonderful_video_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/wonderful_video/set?wonderful_video_id=' + obj["wonderful_video_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "wonderful_video",
							source_field: "wonderful_video_id",
							source_id: this.obj.wonderful_video_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.wonderful_video_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "wonderful_video",
					source_field: "wonderful_video_id",
					source_id: obj["wonderful_video_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.video_name
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/wonderful_video/set?wonderful_video_id=' + _this.obj["wonderful_video_id"], {
								praise_len
							}, (res) => {
								if(res.result){
